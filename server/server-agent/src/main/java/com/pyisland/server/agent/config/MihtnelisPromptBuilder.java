...
public class MihtnelisPromptBuilder {
    ...
    public String buildSystemPrompt(boolean proUser, List<String> workspaces, List<MihtnelisAgentStreamService.SkillEntry> skills, boolean snapshotMode, boolean localMode) {
        ...
        p.append("# 工具使用指南\n")
         ...
         .append("- sys.nowplaying：获取当前正在播放的歌曲信息（通过 Windows SMTC）。无参数。返回 title、artist、album、isPlaying、duration_ms、position_ms、deviceId。不返回专辑封面。低风险无需授权。\n")
        ...
        p.append("示例31 - 用默认程序打开文件：\n")
         .append("{\"type\":\"tool_call\",\"tool\":\"sys.launch\",\"purpose\":\"用默认关联程序打开用户指定的文档\",\"arguments\":{\"target\":\"C:\\\\Users\\\\user\\\\Documents\\\\report.pdf\"}}\n\n");
        ...
        if (snapshotMode) {
            p.append("\n# 快照模式（最高优先级）\n")
             ...
             .append("- **Markdown 仅允许加粗（**文字**）、斜体（*文字*）、删除线（~~文字~~）等简单字体样式。禁止使用标题（#）、列表（-/*/1.）、表格、代码块（```）等复杂排版。**\n")
    ...
    public String buildNativeToolSystemPrompt(boolean proUser, List<String> workspaces, List<MihtnelisAgentStreamService.SkillEntry> skills, boolean snapshotMode) {
        ...
        p.append("# 可用工具\n")
         ...
         .append("系统：sysInfo（OS/CPU/内存）、sysEnv（环境变量查询）、sysOpen（打开 Windows 系统组件）、sysInstalledApps（查询已安装程序列表）、sysLaunch（启动程序/打开文件/URL）、sysNowplaying（当前播放歌曲信息）\n")
        ...
        p.append("# 决策策略\n")
         ...
         .append("- 当前播放：sysNowplaying 获取正在播放的歌曲信息（title/artist/album/isPlaying/duration_ms/position_ms）。不返回专辑封面。\n")
         .append("- eIsland 设置：\n")
